...
export const CANVAS_WIDTH = COLS * (SNAKE_SIZE + GAP_SIZE);
export const CANVAS_HEIGHT = ROWS * (SNAKE_SIZE + GAP_SIZE);
...
function paintCell(ctx: CanvasRenderingContext2D, point: Point2D, color: string) {
  const x = point.x * SNAKE_SIZE + (point.x * GAP_SIZE);
  const y = point.y * SNAKE_SIZE + (point.y * GAP_SIZE);
